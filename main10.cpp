...
void PrizeSet();//没用上
void PrizeInfornation();//没用上
void PrizeNumber(int group[]);//没用上
void single(int group[]);//没用上
void multiple(int group[]);//没用上
void OutputInit();//显示 当前奖项设置 情况
void ChangePri();//添加一个奖项
...
void GetAllData(Student2 student[] );//从文件中读取所有学生数据
void Roll(Student2 students[]);  //滚动它们
unsigned int Rand(int min, int max2);//max2 为不与max重名
void delay(unsigned int xms);//延时函数
int lovelyms=400;
void Lovely();//在有一大波输出时 暂停一小会儿
...
void InputTime(void);  //将时间写入记录文本
void InputPri(int i, Prize pri);  //将奖项名称写入记录文件
void InputStudent(int b, Student2 students[]); //将中奖者信息写入记录文件
void OutputRecord(); //打印抽奖记录
int CountLines(char *filename); //统计文本行数
void OutputOneRecord(); //打印每次抽奖结束后的获奖信息
...
void StudentsSet();//抽奖人信息设置


void PrizeGiftSet();//奖项奖品设置
void PrizeOneChange();//改变单个奖项 名称 人数 奖品
void PrizeDelete();//删除一个奖项
void PrizeSpaceChange();//改变奖项位置
...
void StudentsSet()//张海军 输入抽奖人信息函数
{
    int i;
    ...
    while(1){
        ...
        if(strcmp (s,"1")==0){
            ...
            for(i=0;i<n;i++)
            {
                ...
                if(std::cin>>stus[i].num>>stus[i].name>>stus[i].sex>>stus[i].nj>>stus[i].grjj){
...
void PrizeSpaceChange()//改变两个奖项位置
{
    ...
    printf("**位置编号: %d ** \n",i+1);
    if(N==0)printf("没有奖项！\n");
    printf("请输入你想放入的位置编号(输0退出)\n");
    while(1)
    {
        cin>>str3;
        if(IsNum(str3)){
            if(StrToNum(str3)>=0 && StrToNum(str3)<=N+1) break;//
            else printf("没有这个位置编号，再试一次 \n");
            
        }else{
            printf("没有这个位置编号，再试一次 \n");
        }
    }
...
void Lottery()  //抽奖函数
{
    ...
    int a=0;  //每次抽奖都重置，用于判断 没有更多的候选人了
